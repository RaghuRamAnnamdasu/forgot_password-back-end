...
async function genHashedPassword(password){
    ...
    console.log(userFromDB);
      ...
      //  const secret = process.env.secretKey + userFromDB.password;
       const secret = process.env.secretKey;
        ...
        console.log(link);
        mailer(email,link);
        const result = await client.db("forgotPassword").collection("users").updateOne({email : email},{$set : {resetToken : token}});
        console.log(result);
  ...
  router.post("/resetPassword", async function(req,res){
    ...
    const userDetails = await client.db("forgotPassword").collection("users").find({_id : ObjectId(id)}).toArray();
    ...
    jwt.verify(token,process.env.secretKey,async function(err,decodedData){
      if(err){
        ...
        const result = await client.db("forgotPassword").collection("users").updateOne({_id : ObjectId(id)},{$set : {password : hashedPassword}});
